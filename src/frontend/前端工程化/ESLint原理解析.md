...
当你使用`espree`的时候，就必须遵循它的AST结构，比如`@babel/parser`对于数字`1`, 它的节点类型是`NumericLiteral`, 而`expree`的节点类型是`Literal`。
...
在上面的配置中，只是使用了`node`和`browser`，另外还支持`es20xx`的配置，根据不同的ecma版本来配置。
